...
### Paso 1: Levantar el entorno en SORSABSA

**IMPORTANTE**: PeritoDigital actúa como el procesador lógico de **Conector**. Asegúrate de que el servicio esté declarado en el Docker Global:

```powershell
C:\Sorsabsa\up.sh peritodigital
```

### Paso 2: Acceso a Claude Code (interno)

No instales `claude-code` en tu máquina. El contenedor ya tiene las herramientas necesarias. Para interactuar con el agente:

```powershell
# Sincronización con Conector (Meta Gateway)
PERITODIGITAL_URL=https://peritodigital-production.up.railway.app

C:\Sorsabsa\.docker_global\run.ps1 peritodigital bash
```
